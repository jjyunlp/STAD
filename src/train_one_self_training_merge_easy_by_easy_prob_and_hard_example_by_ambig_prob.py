...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_ambig_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        # 这边ambig也只是用第一个label
        pseudo_ambig_inst, pseudo_ambig_bert = data_processor.get_examples_from_file(
            pseudo_ambig_example_file)
        ...
        # 这边ambig_example也仅仅使用第一个label
        pseudo_ambig_bert = data_processor._create_examples_from_json_insert_entity_tag(pseudo_ambig_inst)
