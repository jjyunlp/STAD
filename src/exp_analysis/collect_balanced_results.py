...
small_data_name = f"balanced_small_data_exp01"
...
if __name__ == "__main__":
    ...
    part_list = [20]
    print("base_e1e2")
    collect_small_data_base_results(dataset_name, exp_base_dir, part_list)
    print("Merge Prob Threshold")
    prob_thresholds = [0.95, 0.9, 0.8, 0.0]
    if True:
        prob_threshold = 0.95
    ...
    if False:
        method = "merge_partial_v2_ambiguity_0.005_0.95"
        prob_thresholds = [0.95]
        for prob_threshold in prob_thresholds:
            print(f"merge_prob={prob_threshold} "),
            for label_num in label_nums:
                method = f"merge_partialXXX_{prob_threshold}"
                print(method)
    ...
    if True:
        ...
        for prob_threshold in prob_thresholds:
            ...
            for label_num in label_nums:
                print(label_num)
                # method = f"merge_ambiguity_partial_loss_max_all_{label_num}_{prob_threshold}"
                method = f"merge_easy_and_hard_by_accumulate_prob{prob_threshold}_top{label_num}"
                method = f"merge_easy_and_ambiguity_hard_by_accumulate_prob{prob_threshold}_top{label_num}"
                print(method)
                collect_base_results(dataset_name, exp_base_dir, part_list, method)
