...
we sum all prob of 0's for ambig2, and then calculate the loss, after that, we avg it by 0's num.

这个就是我们现在用的方法【2021-11-12】
每次仅选取一个随机的negative label作为negative training的目标
退一步，这个只做在hard label上，注意下，flag标记我们还是要用的，因此，onehot_label(partial)要用label(hard)来替换
"""
import random
import os
import numpy as np
...
from data_processor.inst_to_example_converter import SelfTrainingREDataProcessor, SelfTrainingTextDataProcessor
...
from model.bert_model import BertModelOutCLS, BertModelOutCLSPositiveAndNegative, BertModelOutE1E2NegativeSupportPartialBySumLoss
# 为什么这边用的是NegativeSupport呢
# from model.bert_model import BertModelOutCLS, BertModelOutE1E2PositiveAndNegative, BertModelOutE1E2NegativeSupportPartialBySumLoss
...
from method.self_training import Sampling, SelfTrainingTrainAndTest, TextSampling
...
# model_class = BertModelOutE1E2NegativeSupportPartialBySumLoss
# model_class = BertModelOutE1E2PositiveAndNegative
model_class = BertModelOutCLSPositiveAndNegative
...
data_processor = SelfTrainingTextDataProcessor(
...
sampler = TextSampling(id2label, train_inst, drop_NA=args.drop_NA)
model.to(args.device)
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    pseudo_easy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_easy_example_prob{args.easy_prob_threshold}.txt'
    )
    pseudo_hard_example_file = os.path.join(
        current_data_dir,
        f'pseudo_hard_example_accumulate_prob{args.easy_prob_threshold}_top{args.max_label_num}.txt'
    )
    pseudo_noisy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_noisy_example_accumulate_prob{args.easy_prob_threshold}_top{args.max_label_num}.txt'
    )
    # To get above data
    ...
    if os.path.isfile(pseudo_all_file):
        ...
        pseudo_all_inst = trainer.tagging_for_trec_yahoo(args, model, unlabel_tensor, unlabel_inst, id2label)
    ...
    # Merge
    train_inst += (pseudo_easy_inst + pseudo_hard_inst)
    train_bert += (pseudo_easy_bert + pseudo_hard_bert)
    ...
    if not os.path.exists(checkpoint):
        ...
        trainer.train_onehot(args, model, train_tensor, val_tensor, test_tensor,
                      ...
                      use_random_one_negative_training_for_hard_label=True)        # 重要：将hard label amb -> negative
        logging.info("End Training.")
