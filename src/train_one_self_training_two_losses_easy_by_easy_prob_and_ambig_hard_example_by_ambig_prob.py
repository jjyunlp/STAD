"""
2021/8/20, Junjie Yu, Soochow University

easy and ambig prob N-1/N and 1/N
one loss is standard CE loss for gold and easy data
another loss is a ambiguity-support CE loss for ambig data.
"""
import random
import os
import numpy as np
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    pseudo_easy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_easy_example.txt'
    )
    pseudo_ambig_example_file = os.path.join(
        current_data_dir,
        f'pseudo_ambig_example.txt'
    )
    pseudo_noisy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_noisy_example.txt'
    )
    # To get above data
    ...
    # use_cache，用于我们在调整easy/hard/noisy example的时候，覆盖原有的文件
    easy_prob = (num_labels - 1) / num_labels
    ambig_prob = 1 / num_labels
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_ambig_example_file) and os.path.isfile(pseudo_noisy_example_file):
        logging.info(f"Read exist easy example pseudo file from {pseudo_easy_example_file}")
        pseudo_easy_inst, pseudo_easy_bert = data_processor.get_examples_from_file(
            pseudo_easy_example_file)
        logging.info(f"Read exist hard example pseudo file from {pseudo_ambig_example_file}")
        # 这边ambig需要模糊标注的one-hot
        pseudo_ambig_inst, pseudo_ambig_bert = data_processor.get_examples_from_file_with_ambiguous_label(
            pseudo_ambig_example_file, ambig_prob)
        ...
        pseudo_easy_inst, pseudo_ambig_inst, pseudo_noisy_inst = sampler.sample_with_n_labels_larger_than_ambig_prob_threshold(
            pseudo_all_inst, easy_prob, ambig_prob)
        # 转换只用一个label的概率就大于阈值的easy example
        pseudo_easy_bert = data_processor._create_examples_from_json_insert_entity_tag(pseudo_easy_inst)
        # select topN prob that satisfy the prob_threshold from other examples
        pseudo_ambig_bert = data_processor._create_examples_from_json_insert_entity_tag_for_ambig_example(pseudo_ambig_inst, ambig_prob)
        data_processor.dump_data(pseudo_easy_example_file, pseudo_easy_inst)
        data_processor.dump_data(pseudo_ambig_example_file, pseudo_ambig_inst)
    ...
    # Merge
    train_inst += (pseudo_easy_inst + pseudo_ambig_inst)
    train_bert += (pseudo_easy_bert + pseudo_ambig_bert)
    logging.info(f"Epoch {self_train_epoch}'s training size={len(train_inst)}")
    logging.info(f"{len(pseudo_noisy_inst)} noisy inst")
    # 开始训练，直接调用训练baseline的方法
    train_feature = feature_converter.convert_examples_to_features(train_bert, max_length=args.max_seq_length)
    train_tensor = tensor_converter.feature_to_tensor(train_feature)

    ambig_feature = feature_converter.convert_examples_to_features(pseudo_ambig_bert, max_length=args.max_seq_length)
    ...
    if not os.path.exists(checkpoint):
        ...
        # one standard CE loss, one partial-support CE loss (sum of all 1 labels)
