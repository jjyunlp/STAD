"""
2021/8/9, Junjie Yu, Soochow University
...
将loss都改成减少负例的总和，onehot的1/0互换，直接在loss中改就行
...
from model.bert_model import BertModelOutCLS, BertModelOutE1E2WithPartialforNegative
...
model_class = BertModelOutE1E2WithPartialforNegative
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if os.path.isfile(pseudo_select_file) and os.path.isfile(pseudo_unused_file):
        ...
        pseudo_select_inst, pseudo_select_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_select_file, label_num=1)
        ...
        pseudo_unused_inst, pseudo_unused_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_unused_file, args.label_num)
    else:
        pseudo_select_inst, pseudo_unused_inst = sampler.sample_with_prob_threshold(
            pseudo_all_inst, args.prob_threshold)
        # use one label num
        pseudo_select_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial(pseudo_select_inst, label_num=1)
        pseudo_unused_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial(pseudo_unused_inst, args.label_num)
