...
"""
训练基础模型的脚本
这个脚本包括所有，特别是为了重跑sampling新的training data。
...
# global setting
cuda_num = 4
exp_id = 1   # 后续会跑多种小样本，即多次随机切分

prob_thresholds = [0.95, 0.9, 0.85, 0.8]
prob_thresholds = [0.6, 0.5]
...
if dataset_name == 'semeval':
    ...
    max_label_num = 39
server = 'AI'
part = 5   # each for class
if server == '139':
    exp_base_dir = "/data4/jjyunlp/rc_output/self_training_mixup"   # for 139
if server == 'AI':
    if use_micro:
        exp_base_dir = f"/data/jjy/SelfTrainingRE_micro_accumulate_prob_{part}_low_resource"
    if use_macro:
        exp_base_dir = f"/data/jjy/SelfTrainingRE_macro_accumulate_prob_{part}_low_resource"

small_data_name = f"low_resource_exp_{exp_id}"
...
def collect_results(dataset_name, exp_base_dir, method, is_baseline=False, is_two_stage=False, is_two_stage_first=False, iter=0):
    ...
    result_record_file = os.path.join(exp_base_dir, f"{dataset_name}_{part}_results_exp_{exp_id}.txt")
    with open(result_record_file, 'a') as writer:
        # 不停的添加新的实验结果
        writer.write(f"{method} {exp_id}\n")
