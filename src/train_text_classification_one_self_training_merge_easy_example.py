"""
2023/02/17, Junjie Yu, Soochow University
only merge easy example, for sentence classification tasks
"""
import random
import os
import numpy as np
...
from model.bert_model import BertModelOutCLSPositive
...
model_class = BertModelOutCLSPositive
...
sampler = TextSampling(id2label, train_inst)
model.to(args.device)
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_hard_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_hard_inst, args.easy_prob_threshold, topN=1)
