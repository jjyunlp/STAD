...
先easy + ambig2使用negative/positive混合训练，其中negative使用sun loss的方法，
接着再gold使用positive训练

2021/11/3 add iterative for uncertain data
"""
import random
import os
import numpy as np
...
from model.bert_model import BertModelOutCLS, BertModelOutE1E2NegativeSupportPartialBySumLoss
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if os.path.exists(os.path.join(output_dir, 'first', 'results')) and os.path.exists(os.path.join(output_dir, 'second', 'results')):  # and overwrite
        # 方便续跑
        continue
    if self_train_epoch != 0:
        # load model from last epoch as teacher model
        last_dir = os.path.join(args.output_dir, f"epoch_{str(self_train_epoch - 1)}")
        last_checkpoint = os.path.join(last_dir, 'second', checkpoint_name)
        model = model_class.from_pretrained(last_checkpoint)
        model.to(args.device)
    ...
    # 这一个结果应该是上一轮模型的标注结果，如果是第一轮，那就在base中，否则是在上一轮
    # 因此，对于iterative，我们是训练得到最后一轮的结果后，需要再去标注下unlabeled data，得到新的，用于下一轮
    if os.path.isfile(pseudo_all_file):
        ...
        # 如果没有pseudo-labeled data，那就说明需要用上一轮的model重新进行标注
    ...
    if not os.path.exists(result_file):
        ...
        with open(result_file, 'w') as writer:
            json.dump(json_results, writer, indent=2)
    # if iteration, we should to keep model is the last iteration, then use it to tagging.
