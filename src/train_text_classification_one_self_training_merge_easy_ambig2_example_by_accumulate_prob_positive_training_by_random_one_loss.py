...
每次仅选取一个随机的positive label作为npositive training的目标

都是在train_onehot()中的参数控制。
...
from model.bert_model import BertModelOutCLS, BertModelOutE1E2PositiveAndNegative, BertModelOutCLSPositiveAndNegative
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_hard_inst, pseudo_hard_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_hard_example_file, args.easy_prob_threshold, label_num=args.max_label_num)     # 只选用第一个，对比实验。
    ...
    if not os.path.exists(checkpoint):
        ...
        trainer.train_onehot(args, model, train_tensor, val_tensor, test_tensor,
                      ...
                      use_random_one_positive_training=True)
        logging.info("End Training.")
