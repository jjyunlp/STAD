"""
2021/8/16, Junjie Yu, Soochow University
...
Two losses:
one softmax CE loss for gold and easy examples
one softmax CE loss for ambiguous examples.
这个作为softmin CE loss for ambiguous的对照实验。正好也尝试下分开loss的做法。
...
from model.bert_model import BertModelOutE1E2WithPartial, BertModelOutE1E2WithPartialforNegative
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_hard_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_hard_inst, args.prob_threshold, args.max_label_num)
