...
for self_train_epoch in range(args.self_train_epoch):
    ...
    pseudo_unused_file = os.path.join(
        current_data_dir,
        f'pseudo_unused_prob_{args.prob_threshold}_top_{args.max_label_num}.txt'
    )
    # To get above data
    ...
    if os.path.isfile(pseudo_select_file) and os.path.isfile(pseudo_unused_file):
        ...
        pseudo_select_inst, pseudo_select_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_select_file, args.prob_threshold, label_num=1)
        logging.info(f"Read exist unused pseudo file from {pseudo_unused_file}")
        pseudo_unused_inst, pseudo_unused_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_unused_file, args.prob_threshold, args.label_num)
    else:
        pseudo_select_inst, pseudo_unused_inst = sampler.sample_with_prob_threshold(
            pseudo_all_inst, args.prob_threshold)
        # use one label num
        # select只用一个label的概率就大于阈值
        pseudo_select_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_select_inst, args.prob_threshold, topN=1)
        # unsued中挑选topN个了label的概率和大于等于阈值
        pseudo_unused_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_unused_inst, args.prob_threshold, args.max_label_num)
    ...
    # Merge
    train_inst += (pseudo_select_inst + pseudo_unused_inst)
    train_bert += (pseudo_select_bert + pseudo_unused_bert)
    logging.info(f"Epoch {self_train_epoch}'s training size={len(train_inst)}")
    logging.info(f"{len(train_inst)} train inst, and {len(unlabel_inst)} unlabel inst")
