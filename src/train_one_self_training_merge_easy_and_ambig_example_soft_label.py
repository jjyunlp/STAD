...
from model.bert_model import BertModelOutCLS, BertModelOutE1E2Positive
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_easy_inst, pseudo_easy_bert = data_processor.get_examples_from_file_with_soft_label(
        ...
        pseudo_hard_inst, pseudo_hard_bert = data_processor.get_examples_from_file_with_soft_label(
            pseudo_hard_example_file, args.easy_prob_threshold, label_num=1)     # 只选用第一个，对比实验。
        ...
        pseudo_easy_bert = data_processor._create_examples_from_json_insert_entity_tag_with_soft_label_by_accumulate_prob(pseudo_easy_inst, args.easy_prob_threshold, topN=1)
        # select topN prob that satisfy the prob_threshold from other examples
        pseudo_hard_bert = data_processor._create_examples_from_json_insert_entity_tag_with_soft_label_by_accumulate_prob(pseudo_hard_inst, args.easy_prob_threshold, topN=1)
    ...
    # Merge
    # 这个写法在迭代时会出事，还是要另起一个变量名，如current_train_inst = a + b
    ...
    # we only use onehot_label, label is not used
    train_feature = feature_converter.convert_examples_to_features(train_bert, max_length=args.max_seq_length)
    train_tensor = tensor_converter.feature_to_tensor(train_feature)
