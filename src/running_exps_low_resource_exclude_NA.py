...
For Re-TACRED, the NA is the biggest part, which may have bad influence.
Test if exclude NA relation.
还没改：
1. dataset_name
2. Evaluation
3. max_label_num
...
def collect_results(dataset_name, exp_base_dir, method, is_baseline=False, is_two_stage=False, is_two_stage_first=False, iter=0):
    ...
    with open(result_record_file, 'a') as writer:
        ...
        writer.write(f"{test_list}: avg={round(sum(test_list)/len(test_list), 5)}, std={round(np.std(val_list)*100, 1)}\n")
        writer.write("\n")
        
    if is_two_stage_first:
        print("two-stage first train:")
    print(val_list, round(sum(val_list)/len(val_list), 5), round(np.std(val_list)*100, 1))
    print(test_list, round(sum(test_list)/len(test_list), 5), round(np.std(val_list)*100, 1))
...
def do_merge_ablation(max_label_num, easy_prob_threshold, ambig_prob_threshold, alpha=None):
    # 两个剥离实验
...
if __name__ == "__main__":
    ...
    train_self_training_partial_negative = True
    if train_self_training_partial_negative:
        prob_thresholds = [0.95]
        for prob in prob_thresholds:
            do_merge_easy_and_ambig2_training_negative(max_label_num, prob, prob)

    # hard + soft label，都只要跑一次就行
    ...
    train_merge_ablation = False
    if train_merge_ablation:
        do_merge_ablation(max_label_num, prob_threshold, prob_threshold)
