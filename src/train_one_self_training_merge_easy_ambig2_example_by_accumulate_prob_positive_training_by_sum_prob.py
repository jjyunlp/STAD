...
all positive training, sum the probs of 1's class, and avg the loss
"""
import random
import os
import numpy as np
...
from model.bert_model import BertModelOutCLS, BertModelOutE1E2PositiveSupportPartialBySumProb
...
model_class = BertModelOutE1E2PositiveSupportPartialBySumProb
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        # select topN prob that satisfy the easy_prob_threshold from other examples
