...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_hard_inst, pseudo_hard_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_hard_example_file, args.prob_threshold, label_num=1)     # 只选用第一个，对比实验。
        ...
        pseudo_hard_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_hard_inst, args.prob_threshold, topN=1)
    ...
    first_train_inst = pseudo_hard_inst
    first_train_bert = pseudo_hard_bert
    ...
    second_train_inst = train_inst + pseudo_easy_inst
    second_train_bert = train_bert + pseudo_easy_bert
