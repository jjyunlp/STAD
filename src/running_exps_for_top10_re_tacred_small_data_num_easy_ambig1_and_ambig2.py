...
针对re-tacred只选取top-10个大比例的关系，然后unlabel也只保留这些。small data即每个类别取N个
...
dataset_name = 'top10-re-tacred'
exp_id = '01'   # 后续会跑多种小样本，即多次随机切分
part = 50   # each for class
max_label_nums = [2, 3, 4, 5, 7, 8, 9]
max_label_nums = [5]
max_label_num = 5       # for merge easy 占位符一样
small_data_name = f"balanced_small_data_exp{exp_id}"
train_file = f"../data/re-tacred_top10_label_excluding_NA/{small_data_name}/train_{part}.txt"
val_file = f"../data/re-tacred_top10_label_excluding_NA/val.txt"
test_file = f"../data/re-tacred_top10_label_excluding_NA/test.txt"
label_file = f"../data/re-tacred_top10_label_excluding_NA/label2id.json"
unlabel_file = f"../data/re-tacred_top10_label_excluding_NA/{small_data_name}/unlabel_{part}.txt"
...
def run_one_self_training_two_stage_fine_tune(python_file, exp_base_dir, lr_1, lr_2, easy_prob_threshold, ambig_prob_threshold, method=None, iteration=1):
    # dataset
    model_type = "bert"
    model_name_or_path = "../../bert-base-uncased"
    save_steps = 100
...
def run_one_self_training_base_merge_pseudo_data(python_file, exp_base_dir, easy_prob_threshold, ambig_prob_threshold, method, max_label_num=1, alpha=None, iteration=1):
    """[summary]
...
def do_self_training(prob_threshold, iteration=10):
    ...
    if train_merge_easy_and_ambig2_example_positive_sum_prob_then_loss:
        ...
        collect_results(dataset_name, exp_base_dir, method)
    # 把上面的False，这个明天要跑
    train_merge_easy_and_ambig2_example_negative_sum_prob_then_loss = False
...
def do_merge_easy_and_ambig2_training_negative(max_label_num, easy_prob_threshold, ambig_prob_threshold, alpha=None):
    ...
    if one_random_loss:
        print("one random loss for partial label")
...
def do_two_stage_negative_sum_loss(max_label_num, easy_prob_threshold, ambig_prob_threshold, self_training_epoch=10):
    ...
    if train_easy_and_ambig2_to_gold_random_one_partial:
        method = f"two_stage_easy_and_ambig2_to_gold_lr1_{lr_1}_lr2_{lr_2}_prob{easy_prob_threshold}_top{max_label_num}_batch{batch_size}_by_random_one_negative_loss"
...
def do_two_stage(max_label_num, prob_threshold):
    ...
    train_two_stage_ambig2_to_easy_and_gold = False
...
if __name__ == "__main__":
    ...
    alphas = [0.1, 0.3, 0.5]    # control two loss
    alphas = [1.0]
