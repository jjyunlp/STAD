...
two losses in first fine-tuning: easy CE and ambiguous softmin CE
"""
import random
import os
import numpy as np
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    pseudo_easy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_easy_example_prob_{args.prob_threshold}.txt'
    )
    pseudo_hard_example_file = os.path.join(
        current_data_dir,
        f'pseudo_hard_example_accumulate_prob_{args.prob_threshold}_top_{args.max_label_num}.txt'
    )
    pseudo_noisy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_noisy_example_accumulate_prob_{args.prob_threshold}_top_{args.max_label_num}.txt'
    )
    # To get above data
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_hard_inst, pseudo_hard_bert = data_processor.get_examples_from_file_with_partial(
            pseudo_hard_example_file, args.prob_threshold, label_num=args.max_label_num)
    ...
    first_train_inst = pseudo_easy_inst + pseudo_hard_inst
    first_train_bert = pseudo_easy_bert + pseudo_hard_bert
    ...
    pseudo_easy_feature = feature_converter.convert_examples_to_features(pseudo_easy_bert, max_length=args.max_seq_length)
    pseudo_easy_tensor = tensor_converter.feature_to_tensor(pseudo_easy_feature)
    pseudo_hard_feature = feature_converter.convert_examples_to_features(pseudo_hard_bert, max_length=args.max_seq_length)
    pseudo_hard_tensor = tensor_converter.feature_to_tensor(pseudo_hard_feature)

    logging.info(f"Epoch {self_train_epoch}'s first training size={len(pseudo_easy_inst)} VS {len(pseudo_hard_inst)}")
    ...
    if not os.path.exists(checkpoint):
        ...
        trainer.train_onehot_two_losses(args, model, pseudo_easy_tensor, pseudo_hard_tensor, val_tensor, test_tensor,
                             output_dir=checkpoint, use_random_subset=args.use_random_subset,
                             learning_rate=args.learning_rate, batch_size=16)   # batch size要处理下
        logging.info("End Training.")
    ...
    if not os.path.exists(checkpoint):
        ...
        trainer.train_onehot(
                            ...
                            output_dir=checkpoint, use_random_subset=args.use_random_subset,
                            learning_rate=args.learning_rate_2, batch_size=32)   #
        logging.info("End Training.")
