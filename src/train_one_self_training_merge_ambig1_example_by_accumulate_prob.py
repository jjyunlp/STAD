...
only ambig1, no use easy data
ambig1: ambiguous data with single label
"""
import random
import os
import numpy as np
...
for self_train_epoch in range(args.self_train_epoch):
    ...
    pseudo_hard_example_file = os.path.join(
        current_data_dir,
        f'pseudo_hard_example_accumulate_prob{args.prob_threshold}_top{args.max_label_num}_constraint5.txt'
    )
    pseudo_noisy_example_file = os.path.join(
        current_data_dir,
        f'pseudo_noisy_example_accumulate_prob{args.prob_threshold}_top{args.max_label_num}_constraint5.txt'
    )
    # To get above data
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_easy_inst, pseudo_hard_inst, pseudo_noisy_inst = sampler.sample_with_prob_threshold_by_accumulate_with_constraint(
            pseudo_all_inst, args.prob_threshold, args.max_label_num, prob_constraint=5)
        # use one label num
