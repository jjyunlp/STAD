...
for self_train_epoch in range(args.self_train_epoch):
    ...
    if not args.overwrite_cache and os.path.isfile(pseudo_easy_example_file) and os.path.isfile(pseudo_hard_example_file) and os.path.isfile(pseudo_noisy_example_file):
        ...
        pseudo_noisy_bert = data_processor._create_examples_from_json_insert_entity_tag_with_partial_by_accumulate_prob(pseudo_noisy_inst, args.easy_prob_threshold, topN=1)
    ...
    # Merge
    train_inst += pseudo_noisy_inst      # here, hard mean ambig, and noisy mean hard
    train_bert += pseudo_noisy_bert
